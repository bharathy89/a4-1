...
int Page :: Append (Record *addMe) {
	...
	if (curSizeInBytes + ((int *) b)[0] > pageSize) {
		return 0;
	}
...
void File :: GetPage (Page *putItHere, off_t whichPage) {
	putItHere->SetPageLoc(whichPage);
	...
	char *bits = new (std::nothrow) char[putItHere->GetPageSize()];
	if (bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	...
	lseek (myFilDes, putItHere->GetPageSize() * whichPage, SEEK_SET);
	read (myFilDes, bits, putItHere->GetPageSize());
...
void File :: AddPage (Page *addMe, off_t whichPage) {
	...
	if (whichPage >= curLength) {
		...
		for (off_t i = curLength; i < whichPage; i++) {
			int foo = 0;
			lseek (myFilDes, addMe->GetPageSize() * i, SEEK_SET);
	...
	// now write the page
	char *bits = new (std::nothrow) char[addMe->GetPageSize()];
	if (bits == NULL)
	{
		cout << "ERROR : Not enough memory. EXIT !!!\n";
		exit(1);
	...
	addMe->ToBinary (bits);
	//addMe->SetDirty(false);
	lseek (myFilDes, addMe->GetPageSize() * whichPage, SEEK_SET);
	write (myFilDes, bits, addMe->GetPageSize());
	delete [] bits;
#ifdef F_DEBUG
