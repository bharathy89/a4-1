...
public:
	...
	void GenerateBytes(int * bits) {
		int i,j=0;
		bits[0]=numAtts;
		printf("\n");
		for(i=1;i<=2*MAX_ANDS;i++) {
			if(j < MAX_ANDS) {
				bits[i]=whichAtts[j];
				printf("%d %d\n",j,whichAtts[j]);
			} else {
				bits[i]=whichTypes[j -MAX_ANDS];
				printf("%d %d\n",j,whichTypes[j-MAX_ANDS]);
			}
			j++;
		}
...
public:
	...
	int BuildQueryOrder (OrderMaker &left, OrderMaker &right, OrderMaker &center);
